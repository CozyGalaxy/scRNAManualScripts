...
for (i in Assays(data_integrated)){ if(i != "RNA") {data_integrated[[i]] <- NULL}}


if(args$transform == "SCT"){
		data_integrated <- SCTransform(data_integrated, method = "glmGamPoi", vars.to.regress = c("percent.mt", 'nFeature_RNA', 'nCount_RNA'), variable.features.n=nf.usage, verbose=TRUE)
		if(!is.null(args$cc_gene)){
			data_integrated <- CellCycleScoring(data_integrated, s.features = s.genes, g2m.features = g2m.genes, assay = 'SCT', set.ident = TRUE)
    	    data_integrated <- SCTransform(data_integrated, method = "glmGamPoi", vars.to.regress = c("percent.mt",'nFeature_RNA', 'nCount_RNA', "S.Score", "G2M.Score"), variable.features.n=nf.usage, verbose=TRUE)
		}
    ...
    if(!is.null(args$cc_gene)){
        data_integrated <- ScaleData(data_integrated, vars.to.regress = c("percent.mt", 'nFeature_RNA', 'nCount_RNA', "S.Score", "G2M.Score"))
    }else{
        data_integrated <- ScaleData(data_integrated, vars.to.regress = c("percent.mt",  'nFeature_RNA', 'nCount_RNA'))
    }
}
